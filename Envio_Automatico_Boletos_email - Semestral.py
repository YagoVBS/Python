...
lista = pd.read_csv('C:/Users/Usuário/Desktop/Envios de Boletos por Email/Semestral.AGO24.csv')
...
for linha in lista.index:
    ...
    # Preenchimento dos campos de email para copia.
    ...
    pyautogui.press('tab')
    time.sleep(1)    
    pyautogui.write(f"BOLETO SEMESTRAL - CEMITERIO MEMORIAL JARDIM SANTO ANDRE : {contrato}")
    pyautogui.press('enter')
    pyautogui.write("Segue em anexo o boleto Semestral.\n")
    pyautogui.write("\n")
    ...
    try:                                   #PNG da mensagem de erro quando não localiza o anexo
        if pyautogui.locateOnScreen('C:/Users/Usuário/Desktop/Envios de Boletos por Email/Erro1.png'):
            ...
            time.sleep(0.5)
            pyautogui.hotkey('alt', 'f4')       # Salva como rascunho
            time.sleep(0.5)
            pyautogui.press('enter')
            time.sleep(1)
    except pyautogui.ImageNotFoundException: # Caso não encontre a mensagem de erro
        pass
        
    # Comando para enviar o boleto
    time.sleep(2)
